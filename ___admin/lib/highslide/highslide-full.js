...
setClickEvents : function () {
	...
	for (var i = 0; i < els.length; i++) {
		...
		if (type && !els[i].hsHasSetClick) {
			(function(){
				var t = type;
				if (hs2.fireEvent(hs, 'onSetClickEvent', { element: els[i], type: t })) {
...
dim : function(exp) {
	if (!hs2.dimmer) {
		hs2.dimmer = hs2.createElement ('div', {
				...
				owner: '',
				onclick: function() {
					if (hs2.fireEvent(hs, 'onDimmerClick'))
...
keyHandler : function(e) {
	...
	if (!e.target) e.target = e.srcElement; // ie
	if (typeof e.target.form != 'undefined') return true; // form element has focus
	if (!hs2.fireEvent(hs, 'onKeyDown', e)) return true;
	var exp = hs.getExpander();
...
init : function () {
	if (!hs2.container) {
		...
		for (var x in hs2.langDefaults) {
			if (typeof hs[x] != 'undefined') hs2.lang[x] = hs[x];
...
loadHTML : function() {
	...
	if (!hs2.ie || hs2.uaVersion >= 5.5) {
		...
		if (this.iframe) {
			...
			regBody = /(<body[^>]*>|<\/body>)/ig;
			if (regBody.test(s)) s = s.split(regBody)[hs2.ie ? 1 : 2];
...
hs2.addEventListener(document, 'ready', function() {
	if (hs2.expandCursor || hs2.dimmingOpacity) {
		...
		addRule ('.highslide-viewport-size',
			hs2.ie && (hs2.uaVersion < 7 || document.compatMode == 'BackCompat') ?
				'position: absolute; '+
				'left:'+ fix('scrollLeft') +
				'top:'+ fix('scrollTop') +
				'width:'+ fix('clientWidth') +
