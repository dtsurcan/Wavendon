...
class CI_Profiler {
    ...
    $original_dbs= $dbs;
     // echo '_compile_queries  <pre>$dbs::'.print_r( $dbs, true ).'</pre><br>';
    //AppUtils::deb( $dbs, '$dbs::' );
		...
		foreach ($dbs as $db)
		{
			...
			{
        $sql_queries_to_file= '';
        if ( !empty( $this->CI->config->config['sql_queries_to_file'] ) )  {
          $sql_queries_to_file= $this->CI->config->config['sql_queries_to_file'] . strftime( '%Y-%m-%d_%H_%M_%S' ) . /* '___' . microtime(true)  . */ '.txt';
        }
        $WriteToDataBaseArray= array();

				foreach ($db->queries as $key => $val)
				{
					$time = number_format($db->query_times[$key], 4);
          $WriteToDataBaseArray[]= array( 'time'=> $time, 'query'=> $val ); /* nsn */
        ...
        /* if ( !empty($sql_queries_to_file) ) {
          $fd = fopen( $sql_queries_to_file, "a"  );
          foreach( $WriteToDataBaseArray as $WriteToDataBase ) {
            fwrite($fd, 'Time '.$WriteToDataBase['time'].' :'."\n");
            fwrite($fd, $WriteToDataBase['query']."\n\n\n");
          }
          fclose($fd);
        } */
			}
    ...
    // echo '<pre>$this->_available_sections::'.print_r( $this->_available_sections, true ).'</pre><br>';
