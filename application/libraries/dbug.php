...
<style type="text/css">
        ...
        table.dBug_array td.dBug_arrayHeader { background-color:brown; }
...
<?php
  ...
  function makeTableHeader($type,$header,$colspan=2) {
    echo "<table cellspacing=1 cellpadding=1 class=\"dBug_".HtmlSpecialChars($type)."\">
  <tr>
...
//                else echo "<tr><td>".$this->error("array").$this->closeTDRow();
                else echo "<td>".HtmlSpecialChars($this->error("array")).$this->closeTDRow();
                echo "</table>";
        }
        ...
        function varIsObject($var) {
                ...
                if(is_object($var)) {
                        foreach($arrObjVars as $key=>$Val) {
                                ...
                                if(in_array(gettype($value),$this->arrType))
                                        $this->checkType($value);
                                else echo HtmlSpecialChars($value).$this->closeTDRow();
                        }
        ...
        function varIsResource($var) {
                ...
                switch(get_resource_type($var)) {
                        ...
                        case "xml":
                                ...
                                break;
                        default:
                                echo HtmlSpecialChars(get_resource_type($var)).$this->closeTDRow();
                                break;
                }
        ...
        function xmlStartElement($parser,$name,$attribs) {
                ...
                $this->xmlSData[$this->xmlCount].='echo "<strong>'.HtmlSpecialChars($this->xmlName[$this->xmlCount]).'</strong>".$this->closeTDRow();';
        ...
        function xmlEndElement($parser,$name) {
                for($i=0;$i<$this->xmlCount;$i++) {
                        ...
                        echo (!empty($this->xmlCData[$i])) ? HtmlSpecialChars($this->xmlCData[$i]) : "&nbsp;";
                        echo $this->closeTDRow();
                        $this->makeTDHeader("xml","xmlComment");
                        echo (!empty($this->xmlDData[$i])) ? HtmlSpecialChars($this->xmlDData[$i]) : "&nbsp;";
                        echo $this->closeTDRow();
        ...
        function varIsDBResource($var,$db="mysql") {
                ...
                for($i=0;$i<$numfields;$i++) {
                        ...
                        echo "<td class=\"dBug_resourceKey\">".HtmlSpecialChars($field[$i]->name)."</td>";
                }
                echo "</tr>";
                for($i=0;$i<$numrows;$i++) {
                        ...
                        for($k=0;$k<$numfields;$k++) {
                                ...
                                echo "<td>".HtmlSpecialChars($fieldrow)."</td>\n";
                        }
                        echo "</tr>\n";
                }
                echo "</table>";
        ...
        function varIsGDResource($var) {
                ...
                echo HtmlSpecialChars(imagesx($var)).$this->closeTDRow();
                $this->makeTDHeader("resource","Height");
                echo HtmlSpecialChars(imagesy($var)).$this->closeTDRow();
                $this->makeTDHeader("resource","Colors");
                echo HtmlSpecialChars(imagecolorstotal($var)).$this->closeTDRow();
                echo "</table>";
        }
}
?>